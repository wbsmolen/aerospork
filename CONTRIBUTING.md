...
`.debug` level, which the unified log does not persist: `log stream --debug` sees it live and `log
show` does not, so start the stream before reproducing. Note that `com.wbs.aerospork.debug` is
