...
the moment AeroSpork starts. Any other workspace -- one you reach only through a binding of your own,
such as `alt-0 = 'workspace 10'` -- is created when you switch to it and released once it is empty and
not visible.
...
saved — and a save rewrites only the sections you actually edited, so your comments and
formatting survive. A saved change takes effect on screen straight away. Raw TOML is the exception: it applies only when you click Apply, since
