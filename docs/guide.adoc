...
xref:commands.adoc#open-settings[`aerospork open-settings`] (or the menu-bar icon) opens a native
settings window with seven panes: General, Gaps, Keys, Monitors, Events, Window Rules and Raw
TOML. Structured controls apply live, a moment after you stop changing them — there is no Save
button — and a save rewrites only the sections you actually edited, so your comments and
formatting survive. Raw TOML is the exception: it applies only when you click Apply, since
...
on, because otherwise nothing would lead back into Settings.
...
finished after 10 seconds, it exits without it. A crash does not run the cleanup either -- nothing
...
socket coming up, and any CLI command that exited non-zero.
...
Use `com.wbs.aerospork.debug` for a debug build. Note that the `.debug` there names the *build*, not
...
rejected, config warnings, hot-reload problems
...
macOS onto a workspace that was not on screen -- an app activating itself, or the next key window
after a close. Start here for "focus jumped on its own". Focus that follows macOS within what is
already on screen -- you clicked a window, including one on another monitor -- is not recorded. The
...
# for the process to actually go matters -- otherwise `open` just re-activates the old one, still
